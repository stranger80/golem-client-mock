            ...
            if (this.RequestorEventPipelines.ContainsKey(subscriptionId))
            {
                ...
                else
                {
                    var receivedEvents = (await Task<ICollection<MarketRequestorEvent>>.Run<ICollection<MarketRequestorEvent>>(() =>
                    {
                        ...
                        if (pipeline.PipelineQueue.TryTake(out MarketRequestorEvent reqEvent, (int)(float)(timeout * 1000)))
                        {
                    ...
                    {
                        // mark the last processed proposal id in the subscription
                        this.SubscriptionRepository.UpdateLastProposalId(subscriptionId, receivedEvents.Where(ev => ev.EventType == MarketRequestorEvent.MarketRequestorEventType.Proposal).Max(ev => ev.OfferProposal?.InternalId));
